...
print (CYAN+"Bienvenido a mi trivia sobre el espacio")
...
def pregunta(enunciado,op1,op2,op3,op4):#el enunciado es la pregunta a realizar y op son las opciones de la pregunta
  time.sleep(1)
  print ("\n"+enunciado)
  print (MAGENTA+"\na)",op1,"\nb)",op2,"\nc)",op3,"\nd)",op4+RESET)
...
def comprobar (respuesta):
  while respuesta not in ("a","b","c","d","A","B","C","D","ayuda","mosca","gane"):
...
def evaluar(respuesta, mal1, mal2, mal3,comodin,correcto):#mal1,mal2,mal3 indican las respuestas incorrectas
  if respuesta == mal1:
    puntos = random.randint(1, 11)
    print("\nPerdiste",puntos,"puntos")
    puntos = puntuacion - puntos
    print(ROJO+"\nIncorrecto!", nombre, " Debes estudiar más sobre los animales\n" +RESET, "\nTu puntuacion es de", puntos, "puntos\n")
    return puntos
  elif respuesta == mal2:
    puntos = random.randint(0, 11)
    print("\nPerdiste",puntos,"puntos")
    puntos = puntuacion - puntos
    print(ROJO+"\nIncorrecto!", nombre, "Sigue intentando te falta pocas alternativas\n" +RESET, "\nTu puntuacion es de", puntos, "puntos\n")
    return puntos
  elif respuesta == mal3:
    puntos = random.randint(0, 11)
    print("\nPerdiste",puntos,"puntos")
    puntos = puntuacion - puntos
    print(ROJO+"\nIncorrecto!", nombre, "Te pasaste de alternativa\n" +RESET, "\nTu puntuacion es de", puntos, "puntos\n")
    return puntos
  elif respuesta == comodin:
    print(AMARILLO+"\nEsta es una respuesta secreta, automaticamente te dare un puntuacion aleatorio entre 20 y 100, cruza los dedos"+RESET)
    ...
    print(AMARILLO+"\nParece que has encontrado una de las palabras comodin... sin embargo no funciona en esta pregunta, quizas funcione en otra."+RESET)
    puntos = puntuacion
...
while iniciartrivia == True:
  ...
  for cuenta in range(0, 6, +1):
    ...
    time.sleep(0.2)
  #PREGUNTA1
  pregunta("1 Como se le llaman a los animales que caminan sobre sus 4 patas","bipedo","Tripode","Cuadrupedo","Ninguna de las anteriores")  
  respC = input("\nTu respuesta:")
  puntuacion = evaluar(comprobar(respC), 'a', 'b', 'd',comodin(),'c')
  
  #PREGUNTA2
  pregunta("2 Como se le llaman a los animales que caminan sobre dos patas","Voladores","Tripode","Cuadrupedo","Bipedo")
  respC = input("\nTu respuesta:")
  ...
  #PREGUNTA 4
  pregunta("4 Quien fue el primer humano descubierto","Homo habilis","Australopithecus","Homo sapien","Homo sapien sapien")
  respC = input("\nTu Respuesta:")
  
  while respC not in ("a" , "b", "c"  , "d"):
    respC = input( "Debes responder a, b, c o d. Ingresa nuevamente tu respuesta: ")
  if respC == "a":
   print(VERDE+"\nMuy bien"+RESET)
   ...
   puntuacion = puntuacion+2
  print("\nGracias", nombre, "Tu puntuacion en esta Trivia es de", puntuacion,"puntos")
  print("\nDeseas continuar la trivia nuevamente?")
  repetirtrivia=input("Responder con un si, si deseas repetir la trivia, o cualquier tecla si deseas terminarla:") .lower()
  if repetirtrivia== "si":
